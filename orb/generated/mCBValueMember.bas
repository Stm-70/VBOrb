Attribute VB_Name = "mCBValueMember"
...
Public Const TypeId As String = "IDL:omg.org/CORBA/ValueMember:1.0"
...
Public Function TypeCode() As cOrbTypeCode
    ...
    If oRecTC Is Nothing Then
        ...
        oMemSeq.Item(5).name = "type_def"
        Set oMemSeq.Item(5).p_type = oOrb.createInterfaceTc("IDL:omg.org/CORBA/IDLType:1.0", "IDLType")
        Set oMemSeq.Item(6) = New cCBStructMember
        oMemSeq.Item(6).name = "access"
        Set oMemSeq.Item(6).p_type = oOrb.createPrimitiveTc(2) 'VBOrb.TCShort
        Set oMemSeq.Item(6).p_type = oOrb.createAliasTc("IDL:omg.org/CORBA/Visibility:1.0", "Visibility", oMemSeq.Item(6).p_type)
        'Overwrite place holder
        Call oRecTC.setRecTc2StructTc("ValueMember", oMemSeq)
    End If
    Set TypeCode = oRecTC
    Exit Function
ErrRollback:
...
Public Function extractFromAny(ByVal oAny As cOrbAny) As cCBValueMember
    Dim oStruct As cCBValueMember
    Set oStruct = New cCBValueMember
    Call oStruct.initByAny(oAny)
    Set extractFromAny = oStruct
End Function

'Helper, struct.writeMe() -> oAny.initByReadValue()
Public Function cloneAsAny(ByVal oStruct As cCBValueMember) As cOrbAny
    On Error GoTo ErrHandler
    Dim oAny As cOrbAny
