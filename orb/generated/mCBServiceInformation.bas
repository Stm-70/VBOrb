Attribute VB_Name = "mCBServiceInformation"
...
Public Const TypeId As String = "IDL:omg.org/CORBA/ServiceInformation:1.0"
...
Public Function TypeCode() As cOrbTypeCode
    ...
    If oRecTC Is Nothing Then
        ...
        oMemSeq.Item(0).name = "service_options"
        Set oMemSeq.Item(0).p_type = oOrb.createPrimitiveTc(5) 'VBOrb.TCUlong
        Set oMemSeq.Item(0).p_type = oOrb.createAliasTc("IDL:omg.org/CORBA/ServiceOption:1.0", "ServiceOption", oMemSeq.Item(0).p_type)
        Set oMemSeq.Item(0).p_type = oOrb.createSequenceTc(0, oMemSeq.Item(0).p_type)
        Set oMemSeq.Item(1) = New cCBStructMember
        oMemSeq.Item(1).name = "service_details"
        Set oMemSeq.Item(1).p_type = mCBServiceDetail.TypeCode()
        Set oMemSeq.Item(1).p_type = oOrb.createSequenceTc(0, oMemSeq.Item(1).p_type)
        'Overwrite place holder
        Call oRecTC.setRecTc2StructTc("ServiceInformation", oMemSeq)
    End If
    Set TypeCode = oRecTC
    Exit Function
ErrRollback:
...
Public Function extractFromAny(ByVal oAny As cOrbAny) As cCBServiceInformation
    Dim oStruct As cCBServiceInformation
    Set oStruct = New cCBServiceInformation
    Call oStruct.initByAny(oAny)
    Set extractFromAny = oStruct
End Function

'Helper, struct.writeMe() -> oAny.initByReadValue()
Public Function cloneAsAny(ByVal oStruct As cCBServiceInformation) As cOrbAny
    On Error GoTo ErrHandler
    Dim oAny As cOrbAny
