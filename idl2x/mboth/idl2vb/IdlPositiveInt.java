 ...
 *  @author  Martin Both
 */
public class IdlPositiveInt extends IdlIdentifier implements IdlConstType
{
 /** Read a <positive_int_const>
	 *
	 *  @param	idlScope	Information about the surrounding scope
	 *  @param	filePos		File position of the identifier
	 */
 public static IdlPositiveInt readIdlPositiveInt(IdlScope idlScope,
  TxtFilePos filePos)
 { return new IdlPositiveInt(idlScope, filePos);
 }
 /**
	 *  @param	surScope	Surrounding scope, IdlSpecification or null
	 *  @param	filePos		File position of the identifier
	 */
 protected IdlPositiveInt(IdlScope surScope, TxtFilePos filePos)
 ...
 { IdlConstValue iConstValue= IdlConstValue.readConstExpression(this,
  ...
  { throw new TxtReadException(tRef.value.getFilePos(),
    "Type mismatch. Constant value must match positive integer type.");
  }
  if(iConstValue.getLong().longValue() <= 0
   || iConstValue.getLong().longValue() > Integer.MAX_VALUE)
  { throw new TxtReadException(tRef.value.getFilePos(),
    "Value is " + iConstValue.getLong().toString()
    + ". Constant value must match positive integer type.");
  }
 ...
 { IdlConstValue iConstValue= IdlConstValue.readConstExpression(this,
  ...
  { throw new TxtReadException(tRef.value.getFilePos(),
    "Type mismatch. Constant value must match integer type.");
  }
  return iConstValue;
 }
 /** (IdlDefinition)
 ...
 { return VbClsWriter.LONG; // USIGNED LONG is LONG in VB
 }
 /** (IdlType)
	 *
   ...
   throws java.io.IOException
 {
  ...
  // mCB.tk_ulong = 5
  out.writeLine("oOrb.createPrimitiveTc(5) 'VBOrb.TCUlong");
 }
 /** (IdlType)
   ...
   throws java.io.IOException
 {
  ...
  out.writeLine(VbWriter.OANY + "." + "getUlong" + "()");
 }
 /** (IdlType)
