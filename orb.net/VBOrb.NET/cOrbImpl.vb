...
<System.Runtime.InteropServices.ProgId("cOrbImpl_NET.cOrbImpl")> Public Class cOrbImpl
	...
	Private Const debuglog As String = "c:\orb.log"
	...
	Private Declare Function dllSelect Lib "WS2_32.dll" Alias "select" (ByVal nfds As Integer, ByRef readfds As FD_SET, ByRef writefds As FD_SET, ByRef exceptfds As FD_SET, ByRef timeout As TIMEVAL) As Integer
...
ErrHandler: 
	...
	Private Sub ConnIncCnt()
		...
		If iLogLevel >= 4 Then
			Call mVBOrb.logMsg(debuglog, "I One more ORB connection, now: " & lOrbConnCnt)
		End If
		Exit Sub
ErrHandler: 
	...
	Public Function ConnGet(ByRef SameProcess As Boolean, ByVal ConnId As Integer, ByRef AutoOpen As Boolean) As cOrbSocket
		...
		Else
			...
			If newSocket.socketHost = oOrbLPnts(0).sOAHost And newSocket.socketPort = oOrbLPnts(0).sOAPort Then
				If iLogLevel >= 4 Then
					Call mVBOrb.logMsg(debuglog, "I c" & CStr(ConnId + 1) & " Open ORB internal connection")
				End If
			...
			Else
				If iLogLevel >= 4 Then
					Call mVBOrb.logMsg(debuglog, "I c" & CStr(ConnId + 1) & " Open ORB connection to " & newSocket.socketHost & ":" & newSocket.socketPort)
				End If
...
ErrHandler: 
	...
	Private Function ConnIsOpen(ByVal ConnId As Integer, ByVal lTime As Integer) As Boolean
		...
		Exit Function
ErrLog:
		Call mVBOrb.logErr(debuglog, "Orb.ConnIsOpen")
		Resume Next
ErrHandler: 
	...
	Public Sub ConnClose(ByVal ConnId As Integer, ByVal ReqCancelType As Short)
		...
		If oOrbConns(ConnId).bSameProcess Then
			If iLogLevel >= 4 Then
				Call mVBOrb.logMsg(debuglog, "I c" & CStr(ConnId + 1) & " Close ORB internal connection")
			End If
			oOrbConns(ConnId).bSameProcess = False
		ElseIf Not oOrbConns(ConnId).oSock Is Nothing Then 
			If iLogLevel >= 4 Then
				Call mVBOrb.logMsg(debuglog, "I c" & CStr(ConnId + 1) & " Close ORB socket connection")
			End If
		...
		Exit Sub
ErrLog:
		Call mVBOrb.logErr(debuglog, "ConnClose")
		Resume Next
	End Sub
	...
	Private Sub ConnSendCloseAndClose(ByVal ConnId As Integer, ByVal ReqCancelType As Short)
		...
		Exit Sub
ErrLog:
		Call mVBOrb.logErr(debuglog, "Orb.ConnSendCloseAndClose")
		Resume Next
	End Sub
	...
	Private Sub ConnSendMessageError(ByVal ConnId As Integer)
		...
		If oOrbConns(ConnId).oSock.isOpen Then
			...
			'6 = MessageError
			Call mVBOrb.logMsg(debuglog, "E c" & CStr(ConnId + 1) & " Send MessageError")
	...
	Private Sub ConnSendCloseConnection(ByVal ConnId As Integer)
		...
		If oOrbConns(ConnId).oSock.isOpen Then
			...
			If (oOrbConns(ConnId).oSock.isIncoming Or iGIOPVersion >= &H102) And Not VisiWorkaround Then 'fix for Visi5 by Andreas Roth
				...
				'5 = CloseConnection
				If iLogLevel >= 4 Then
					Call mVBOrb.logMsg(debuglog, "I c" & CStr(ConnId + 1) & " Send CloseConnection")
				End If
		...
		Select Case msgType
			Case 0 '= GIOP Request received
				...
				If oReqst.initInRequest(Me, lReqstId, oIn) Then 'If oIn.isComplete() Then
					If iLogLevel >= 8 Then
						Call mVBOrb.logMsg(debuglog, "D c" & CStr(ConnId + 1) & " Request '" & oReqst.Operation & "' received")
					End If
				...
				Else
					...
					If iLogLevel >= 8 Then
						If oIn.isComplete() Then
							Call mVBOrb.logMsg(debuglog, "D c" & CStr(ConnId + 1) & " Reply '" & oReqst.Operation & "' received")
						End If
					End If
				End If
			...
			Case 5 '5 = GIOP CloseConnection received
				If iLogLevel >= 4 Then
					Call mVBOrb.logMsg(debuglog, "I CloseConnection message received")
				End If
				Call ConnClose(ConnId, msgType) 'Is deleting oOrbConns().oReqsOI
				Call oIn.destroy()
			Case 6 '= MessageError received
				If iLogLevel >= 2 Then
					Call mVBOrb.logMsg(debuglog, "W MessageError message received")
				End If
				Call ConnClose(ConnId, msgType) 'Is deleting oOrbConns().oReqsOI
				Call oIn.destroy()
			Case 7 '= Fragment received
				If iGIOPVersion < &H102 Then
					Call mVBOrb.logMsg(debuglog, "E Unsupported GIOP " & mVBOrb.GIOPVersion2Str(iGIOPVersion) & " fragment message received")
					...
					Else
						If oReqst.addFragment(oIn) Then 'If oIn.isComplete() Then
							If oReqst.isIncoming() Then
								If iLogLevel >= 8 Then
									Call mVBOrb.logMsg(debuglog, "D c" & CStr(ConnId + 1) & " Fragmented Request '" & oReqst.Operation & "' received")
								End If
								oOut = oReqst.replyRequest()
								If Err.Number <> 0 Then Call mVBOrb.ErrSave() : GoTo ErrLoad2
								If Not oOut Is Nothing Then
							...
							Else
								If iLogLevel >= 8 Then
									Call mVBOrb.logMsg(debuglog, "D c" & CStr(ConnId + 1) & " Fragmented Reply '" & oReqst.Operation & "' received")
								End If
							End If
						End If
					End If
			...
			Case 2005 'received len is 0
				If iLogLevel >= 2 Then
					Call mVBOrb.logMsg(debuglog, "W Empty message received")
				End If
				Call ConnClose(ConnId, msgType) 'Is deleting oOrbConns().oReqsOI
				Call oIn.destroy()
			Case Else
...
ErrLog:
		Call mVBOrb.logErr(debuglog, "ConnRecvMsg")
		Resume Next
	End Sub
...
ErrHandler: 
		Call mVBOrb.ErrSave()
		If iLogLevel >= 4 Then
			Call mVBOrb.logMsg(debuglog, "I " & Err.Description)
		End If
		Resume ErrLoadRaise
ErrLoadRaise: 
		On Error Resume Next
...
ErrHandler: 
	...
	Public Sub performWork(Optional ByVal lWaitTime As Integer = 0)
		...
		ElseIf iOrbState = ORBST_ISINIT Then 
			...
			If iLogLevel >= 4 Then
				Call mVBOrb.logMsg(debuglog, "I ORB is listening on port " & oOrbLPnts(0).sOAPort & " at " & oOrbLPnts(0).sOAHost)
			End If
		...
		Dim readfds As New FD_SET
		Dim writefds As New FD_SET
		Dim exeptfds As New FD_SET

		readfds.fd_count = lFDCnt
		writefds.fd_count = 0
		exeptfds.fd_count = 0
		...
		If dllSelect(0, readfds, writefds, exeptfds, timeOut) = -1 Then
		...
		For slotCnt = 0 To lOrbConnCnt - 1
			...
			If Not oOrbConns(slotCnt).oSock Is Nothing Then
				...
				Else 'recvAgain
					...
					If Err.Number <> 0 Then
						Call mVBOrb.logErr(debuglog, sFuncName) 'Is calling Err.Clear()
			...
			If Not oOrbLPnts(lLPnt).oSock Is Nothing Then
				...
				If recvAgain Then
					...
					If iLogLevel >= ORBLL_INFO Then
						Call mVBOrb.logMsg(debuglog, "I c" & CStr(lFreeSlot + 1) & " Accept new ORB connection from " & oOrbConns(lFreeSlot).oSock.socketHost & ":" & oOrbConns(lFreeSlot).oSock.socketPort)
					End If
				End If
			End If
		Next lLPnt
...
RestOfShutdown: 
		...
		If lOpenCnt <= 0 And iOrbState = ORBST_SHUTDOWN Then
			...
			End If
			'Shutdown completed
			Call mVBOrb.logMsg(debuglog, "ORB shutdown completed")
