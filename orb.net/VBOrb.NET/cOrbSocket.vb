...
#End If
	...
	Private Declare Function dllRecv Lib "WS2_32.dll" (ByVal s As Integer, ByVal buf As String, ByVal length As Integer, ByVal flags As Integer) As Integer
	...
	Private Declare Function dllSend Lib "WS2_32.dll" (ByVal s As Integer, ByVal buf As String, ByVal length As Integer, ByVal flags As Integer) As Integer
...
#End If
	...
	Public Function getHostName(ByVal dotStyle As Boolean) As String
		...
		Dim sHostName As String 'New VB6.FixedLengthString(256)
		'Dim llen As Long
		'llen = 256
		'Call GetComputerName(sHostName, llen)
		'getHostName= Left$(sHostName, llen)
		'If dllHostName(sHostName.Value, 256) <> 0 ThenSystem.Net.Dns.GetHostName
...
ErrHandler: 
	...
	Private Function inet_a2n(ByVal Host As String) As Integer
		...
		' do not understand the ,agic with rect dll call
		Debug.Assert(True)
		...
		End If
		Call dllMoveMemory(lPntr, hent.h_addr_list, 4)
		Call dllMoveMemory(lPntr, lPntr, 4)
		inet_a2n = lPntr
		Exit Function
ErrHandler: 
...
ErrHandler: 
	...
	Public Sub sendString(ByRef str_Renamed As String)
		On Error GoTo ErrHandler
		If dllSend(sockFd, str_Renamed, Len(str_Renamed), 0) <> Len(str_Renamed) Then
...
ErrHandler: 
	...
	Public Function recvString() As String
		...
		Else
			...
			recvString = System.Text.UnicodeEncoding.Unicode.GetString(buffer)
		End If
		Exit Function
ErrHandler: 
	...
	Public Sub initBind(ByVal Host As String, ByVal Port As String)
		...
		If Left(Port, 1) = "+" Then
			lOpt = 1
			Debug.Assert(True) ' need to verify lenb fuctioanlity
			'UPGRADE_ISSUE: LenB function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
			If dllSetSockOpt(sockFd, SOL_SOCKET, SO_REUSEADDR, lOpt, Len(lOpt)) <> 0 Then
		...
		'UPGRADE_ISSUE: LenB function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
		If dllBind(sockFd, sockAddr, Len(sockAddr)) <> 0 Then
		...
		'UPGRADE_ISSUE: LenB function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
		addrLen = Len(sockAddr)
		Debug.Assert(True) ' need to chck len
		'UPGRADE_ISSUE: LenB function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
		If dllGetSockName(sockFd, sockAddr, addrLen) <> 0 Or addrLen <> Len(sockAddr) Then
...
ErrHandler: 
	...
	Public Sub initAccept(ByVal oSocket As cOrbSocket)
		...
		Dim addrLen As Integer
		'UPGRADE_ISSUE: LenB function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
		Debug.Assert(True) ' need once again to check len
		addrLen = Len(sockAddr)
