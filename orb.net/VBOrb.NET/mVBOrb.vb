...
#Const DebugMode = 1
	...
	Private oGlobalEx As _cOrbException
	
	'GetTickCount();
	Public Declare Function dllGetTickCount Lib "kernel32"  Alias "GetTickCount"() As Integer
	...
	Private Declare Sub dllGetSystemTime Lib "kernel32"  Alias "GetSystemTime"(ByRef lpSystemTime As SYSTEMTIME)
	
#If DebugMode Then
	Private lClassDebugIDPool As Integer
#End If
	
	Private lUniqueIDPool As Integer
	
	Private Structure tErr
		Dim lNumber As Integer
		Dim sDescription As String
		Dim sSource As String
	End Structure
	Private aErrs(9) As tErr
	Private lErrIdx As Integer
	
	Public Const MinorOMGVMCID As Integer = &H4F4D0000
	
	Public Const CompletedYES As Integer = 0
	Public Const CompletedNO As Integer = 1
	Public Const CompletedMAYBE As Integer = 2
	
	Public Const ITF_E_UNKNOWN_NO As Integer = &H40200
	Public Const ITF_E_BAD_PARAM_NO As Integer = &H40201
	Public Const ITF_E_NO_MEMORY_NO As Integer = &H40202
	Public Const ITF_E_IMP_LIMIT_NO As Integer = &H40203
	Public Const ITF_E_COMM_FAILURE_NO As Integer = &H40204
	Public Const ITF_E_INV_OBJREF_NO As Integer = &H40205
	Public Const ITF_E_NO_PERMISSION_NO As Integer = &H40206
	Public Const ITF_E_INTERNAL_NO As Integer = &H40207
	Public Const ITF_E_MARSHAL_NO As Integer = &H40208
	Public Const ITF_E_INITIALIZE_NO As Integer = &H40209
	Public Const ITF_E_NO_IMPLEMENT_NO As Integer = &H4020A
	Public Const ITF_E_BAD_TYPECODE_NO As Integer = &H4020B
	Public Const ITF_E_BAD_OPERATION_NO As Integer = &H4020C
	Public Const ITF_E_NO_RESOURCES_NO As Integer = &H4020D
	Public Const ITF_E_NO_RESPONSE_NO As Integer = &H4020E
	Public Const ITF_E_PERSIST_STORE_NO As Integer = &H4020F
	Public Const ITF_E_BAD_INV_ORDER_NO As Integer = &H40210
	Public Const ITF_E_TRANSIENT_NO As Integer = &H40211
	Public Const ITF_E_FREE_MEM_NO As Integer = &H40212
	Public Const ITF_E_INV_IDENT_NO As Integer = &H40213
	Public Const ITF_E_INV_FLAG_NO As Integer = &H40214
	Public Const ITF_E_INTF_REPOS_NO As Integer = &H40215
	Public Const ITF_E_BAD_CONTEXT_NO As Integer = &H40216
	Public Const ITF_E_OBJ_ADAPTER_NO As Integer = &H40217
	Public Const ITF_E_DATA_CONVERSION_NO As Integer = &H40218
	Public Const ITF_E_OBJECT_NOT_EXIST_NO As Integer = &H40219
	Public Const ITF_E_TRANSACTION_REQUIRED_NO As Integer = &H40220
	Public Const ITF_E_TRANSACTION_ROLLEDBACK_NO As Integer = &H40221
	Public Const ITF_E_INVALID_TRANSACTION_NO As Integer = &H40222
	Public Const ITF_E_INV_POLICY_NO As Integer = &H40223
	Public Const ITF_E_CODESET_INCOMPATIBLE_NO As Integer = &H40224
	Public Const ITF_E_REBIND_NO As Integer = &H40225
	Public Const ITF_E_TIMEOUT_NO As Integer = &H40226
	Public Const ITF_E_TRANSACTION_UNAVAILABLE_NO As Integer = &H40227
	Public Const ITF_E_TRANSACTION_MODE_NO As Integer = &H40228
	Public Const ITF_E_BAD_QOS_NO As Integer = &H40229
	Public Const ITF_E_USER_EX As Integer = &H402FF
...
#If DebugMode Then
	...
	Public Function getNextClassDebugID() As Integer
...
#End If
	...
	'ORB Native Code Set
	Public ReadOnly Property ONCSC() As Integer
		Get
			ONCSC = lONCS_C
		End Get
	End Property
	
	Public ReadOnly Property ONCSW() As Integer
		Get
			ONCSW = lONCS_W
		End Get
	End Property
	...
	Public ReadOnly Property VBOrb() As cVBOrb
		Get
			VBOrb = oVBOrb
		End Get
	End Property
	...
	Public Function getLongDiff(ByVal l1 As Integer, ByVal l2 As Integer) As Integer
	...
	Public Function GIOPVersion2Str(ByVal GIOPVersion As Short) As String
		Select Case GIOPVersion
			...
			Case Else
				...
				Call mVBOrb.VBOrb.raiseBADPARAM(8, VBOrb.CompletedNO, "Invalid GIOP version " & CStr(GIOPVersion))
		End Select
	End Function
	
	'ORB Initialization (is called by VBOrb.init, see cVBOrb.init and cOrbImpl.init)
	Public Function init(ByVal ORBId As String, ByVal OAHost As String, ByVal OAPort As String, ByVal OAVersion As Short, ByVal ORBDefaultInitRef As String, ByVal ORBInitRef As String, ByVal LogFile As String, ByVal LogLevel As Short, ByVal VisiWorkaround As Boolean) As cOrbImpl
		If ORBId <> "" Then
			Call mVBOrb.VBOrb.raiseNOIMPLEMENT(1, VBOrb.CompletedNO, "Different ORBIds are not supported")
		End If
		If Not oOrbs Is Nothing Then
			If oOrbs.isDown() Then
				'UPGRADE_NOTE: Object oOrbs may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
				oOrbs = Nothing
			End If
		End If
		Dim oNewOrb As cOrbImpl
		If oOrbs Is Nothing Then
			oNewOrb = New cOrbImpl
			Call oNewOrb.init(ORBId, OAHost, OAPort, OAVersion, ORBDefaultInitRef, ORBInitRef, LogFile, LogLevel, VisiWorkaround)
			oOrbs = oNewOrb
		End If
		init = oOrbs
	End Function
	...
	Public Function defaultOrb() As cOrbImpl
		...
		End If
		If oOrbs Is Nothing Then
			Call mVBOrb.VBOrb.raiseBADINVORDER(0, VBOrb.CompletedNO, "Initialize an ORB before using it")
		End If
		defaultOrb = oOrbs
	End Function
	...
	Public Sub logException(ByRef sLogFile As String, ByVal oUserEx As _cOrbException)
		If Len(sLogFile) > 0 Then
			Call logMsg(sLogFile, "Exception: " & oUserEx.Source & ", " & oUserEx.Description)
		End If
	End Sub
	...
	Public Sub logErr(ByRef sLogFile As String, ByRef SourcePrefix As String)
		...
		If Len(sLogFile) > 0 Then
			...
			End If
			Call logMsg(sLogFile, "Error: " & sNumberStr & ", " & IIf(Err.Source = "", SourcePrefix, SourcePrefix & ":" & Err.Source) & ", " & Err.Description)
		End If
		Call Err.Clear()
	End Sub
	...
	Public Sub logMsg(ByRef sLogFile As String, ByRef sMsg As String)
		...
		If Len(sLogFile) > 0 Then
			iFileNo = FreeFile
			FileOpen(iFileNo, sLogFile, OpenMode.Append)
			PrintLine(iFileNo, getDateTimeStr() & " " & sMsg)
			FileClose(iFileNo)
		End If
	End Sub
	...
	Public Function getDateTimeStr() As String
		...
		getDateTimeStr = CStr(lpSystemTime.wYear) & "-" & format00(lpSystemTime.wMonth) & "-" & format00(lpSystemTime.wDay) & " " & format00(lpSystemTime.wHour) & ":" & format00(lpSystemTime.wMinute) & ":" & format00(lpSystemTime.wSecond)
	End Function
	
	'Is equal to Format(iVal, "00")
	Private Function format00(ByVal iVal As Short) As String
		format00 = CStr(iVal)
	...
	Public Sub ErrReraise(ByRef SourcePrefix As String, Optional ByRef PostDescr As String = "")
		If (Err.Number And vbObjectError) = vbObjectError Then
			Call Err.Raise(Err.Number, IIf(Err.Source = "", SourcePrefix, SourcePrefix & ":" & Err.Source), IIf(PostDescr = "", Err.Description, Err.Description & ", " & PostDescr), "", 0)
	...
	Public Sub ErrLoad()
		...
		Call Err.Raise(aErrs(lErrIdx).lNumber, aErrs(lErrIdx).sSource, aErrs(lErrIdx).sDescription)
	End Sub
	
	Private Function lookupITF(ByRef ExceptionId As String) As Integer
		...
		Else
			...
			On Error Resume Next
			'UPGRADE_WARNING: Couldn't resolve default property of object collITF.Item(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	...
	Public Sub raiseSystemException(ByVal Number As Integer, ByVal id As String, ByVal minor As Integer, ByVal completed As Integer, Optional ByRef PostDescr As String = "", Optional ByVal Source As String = "")
		Dim Description As String
		Description = "CORBA System Exception: [" & id & "] minor code [" & CStr(minor) & "]"
		...
		End Select
		Call mVBOrb.ErrRaise(Number, IIf(PostDescr = "", Description, Description & ", " & PostDescr), Source)
	End Sub
	
	'Helper
	Public Sub readRaiseSystemEx(ByVal oIn As cOrbStream, Optional ByRef PostDescr As String = "")
		On Error GoTo ErrHandler
		'GIOP::SystemExceptionReplyBody
		Dim ExceptionId As String 'string exception_id;
		Dim MinorCodeValue As Integer 'unsigned long minor_code_value;
		Dim CompletionStatus As Integer 'unsigned long completion_status;
		...
		If lITF <= 0 Then
			...
			Call VBOrb.raiseUNKNOWN(2, CompletionStatus, "Undefined CORBA System Exception: [" & ExceptionId & "] minor code [" & CStr(MinorCodeValue) & "]")
		Else
			Call raiseSystemException(lITF, ExceptionId, MinorCodeValue, CompletionStatus, PostDescr)
		End If
		Exit Sub
ErrHandler: 
	...
	Public Sub ErrWriteSystemEx(ByVal oOut As cOrbStream)
		...
		Dim lMinor As Integer 'MinorCodeValue
		Dim lCompleted As Integer 'CompletionStatus
		...
		Else
			...
			Else
				lStart = lStart + 1
				lMinor = CInt(Mid(sDescription, lStart, lEnd - lStart))
				If InStr(lEnd, sDescription, "[YES]", CompareMethod.Text) > 0 Then
					lCompleted = CompletedYES
				ElseIf InStr(lEnd, sDescription, "[NO]", CompareMethod.Text) > 0 Then 
...
ErrHandler: 
	...
	Public Function ErrIsSystemEx() As Boolean
		If (Err.Number And (&H40200 Or vbObjectError)) = (&H40200 Or vbObjectError) Then
	...
	Public Function ErrIsUserEx() As Boolean
		If (Err.Number And (&H402F0 Or vbObjectError)) = (&H402F0 Or vbObjectError) Then
	...
	Public Sub raiseUserEx(ByVal oEx As _cOrbException)
		oGlobalEx = oEx
		Call mVBOrb.ErrRaise(ITF_E_USER_EX Or vbObjectError, (oEx.Description), (oEx.Source))
	End Sub
	
	'Get a CORBA user exception and delete the VBOrb.DLL ErrObject
	Public Function getException() As _cOrbException
		getException = oGlobalEx
		'UPGRADE_NOTE: Object oGlobalEx may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
		oGlobalEx = Nothing
	...
	Public Sub string2ObjKey(ByRef keyStr As String, ByRef objKey() As Byte)
		...
		If Len(keyStr) > 0 Then
			...
			ReDim Preserve objKey(keyLen - 1)
		Else
			'UBound(objKey) = -1
			...
			'objKey = System.Text.UnicodeEncoding.Unicode.GetBytes(MidB(keyStr, 1, 0))
			Debug.Assert(True) ' this was strange in the original vb 6 code cannot convert a zero lenth string... failing
		End If
	End Sub
	...
	'corbaloc  ;/:?@&=+$,-_!~*’()
	'corbaname ;/:?@&=+$,-_.!~*’()
	Private Function bytes2String(ByRef bytes() As Byte, ByRef notEscaped As String) As String
		bytes2String = ""
		Dim iKey As Short
		Dim bChr As Byte
		For iKey = LBound(bytes) To UBound(bytes)
			bChr = bytes(iKey)
			If (bChr >= Asc("0") And bChr <= Asc("9")) Or (bChr >= Asc("A") And bChr <= Asc("Z")) Or (bChr >= Asc("a") And bChr <= Asc("z")) Then
	...
	Public Function nameStr2nameUrl(ByRef nameStr As String) As String
		...
		If Len(nameStr) > 0 Then
			ReDim baName(Len(nameStr) - 1)
			For strPos = 1 To Len(nameStr)
				baName(strPos - 1) = Asc(Mid(nameStr, strPos))
			Next
		Else ' case of zero lenth string cannot convert failingl
			'UBound(baName) = -1
			'UPGRADE_ISSUE: MidB function is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
			'UPGRADE_TODO: Code was upgraded to use System.Text.UnicodeEncoding.Unicode.GetBytes() which may not have the same behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="93DD716C-10E3-41BE-A4A8-3BA40157905B"'
			' baName = System.Text.UnicodeEncoding.Unicode.GetBytes(MidB(nameStr, 1, 0))
			Debug.Assert(True)
		End If
	...
	Public Function sunInitGetNameService(ByVal SunInitRef As cOrbObjRef) As _cOrbObject
		On Error GoTo ErrHandler
		Dim oRequest As cOrbRequest
		oRequest = SunInitRef.Request("get", False)
		...
		oIn = oRequest.OutRes
		sunInitGetNameService = oIn.readObject()
		Exit Function
ErrHandler: 
		'UPGRADE_NOTE: Object sunInitGetNameService may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
		sunInitGetNameService = Nothing
	...
	Public Function nameContextResolveName(ByVal NmCtxRef As cOrbObjRef, ByRef name As String) As _cOrbObject
		On Error GoTo ErrHandler
		'resolve() raises(c_NmNotFoundEx, c_NmCannotProceedEx, c_NmInvalidNameEx)
		Dim oRequest As cOrbRequest
		oRequest = NmCtxRef.Request("resolve", False)
		Dim oOut As cOrbStream
		oOut = oRequest.InArg
		...
		If oRequest.invokeReqst(True) Then
			...
			Select Case sTypeId
				...
				Case Else
					Call VBOrb.raiseUNKNOWN(1, VBOrb.CompletedMAYBE, "Undefined CORBA User Exception: [" & sTypeId & "]")
			End Select
			Call VBOrb.raiseBADPARAM(10, VBOrb.CompletedNO, sErrMsg)
		Else
			oIn = oRequest.OutRes
			nameContextResolveName = oIn.readObject()
		End If
		Exit Function
ErrHandler: 
...
ErrHandler: 
	...
	Private Function nameServiceNameRead(ByVal oIn As cOrbStream) As String
		...
		If seqLen < 0 And seqLen > 10000 Then
			Call VBOrb.raiseMARSHAL(1, VBOrb.CompletedNO, "SeqLen is out of range: " & CStr(seqLen))
		End If
		Dim name As String
		Dim nameId As String
		Dim nameKind As String
...
ErrHandler: 
	...
	Public Function TypeIdEquals(ByRef sTypeId1 As String, ByRef sTypeId2 As String, ByVal bExact As Boolean) As Boolean
	...
	Public Function TypeIdIsInVal(ByRef sTypeId As String, ByVal oValueBase As _cOrbValueBase, ByVal bExact As Boolean) As Boolean
		Dim iIdCnt As Short
